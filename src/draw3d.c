...
static void	calc_wall_slice(t_game *game, double perp_dist, t_slice *s)
{
	...
	s->start = s->mid_y - (int)(s->height / 2);
	s->end = s->mid_y + (int)(s->height / 2);
...
static void	draw_wall_col(t_game *game, int draw_x, int side, t_slice *s)
{
	put_line(game, LINE(
			draw_x,
			game->cont3d.y1,
...
void	draw_3d_view(t_game *game, double init_angle)
{
	...
	while (col < game->nb_rays - 1)
	{
		...
		draw_wall_col(game, game->cont3d.x1 + col, side, &s);
		col++;
	}
}
